...
import { RESULTS_PER_PAGE_OPTIONS, SIZE_OPTIONS, SORT_OPTIONS } from '@nx-bridge/constants';
import { AppState, SetResultsPerPagePreference, SetSortingPreference } from '@nx-bridge/store';
import { LocalStorageManagerService } from '../services/local-storage-manager.service';
import { SearchService } from '../services/search.service';
import { dealsListButtonFontSizeCssPropName, gameDetailHeightAboveBreakpointCssPropName, gameDetailHeightBelowBreakpointCssPropName, gameDetailSummaryHeightPercentageCssPropName, getNewBatchNumber, getNewTotalNumberOfPages, playerLabelsDisplayTypeCssPropName, playerNamesDisplayTypeCssPropName } from '@nx-bridge/constants';
import { GameDetailDisplayPreferences } from '@nx-bridge/interfaces-and-types';
import { gameDetailSizes } from '@nx-bridge/computed-styles';


@Component({
  selector: 'nx-bridge-options',
  templateUrl: './options.component.html',
  styleUrls: ['./options.component.scss']
})
export class OptionsComponent implements OnInit {
  @HostBinding('class.options') get classname() {return true};
